  ...
  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", background: C.cream, minHeight: "100vh" }}>
      ...
      <div
        ...
        style={{ background: C.white, borderBottom: `1px solid ${C.gold}20` }}
      >
        <div className="flex items-center gap-3 sm:gap-5">
          <Link to="/" className="flex h-8 w-8 items-center justify-center rounded-full font-bold text-[13px]" style={{ background: C.darkGreen, color: C.gold }}>
            V
          </Link>
          <span className="hidden sm:block text-[15px] font-bold" style={{ color: C.ink }}>
            {productInfo.name}
          </span>
          <div className="hidden sm:flex items-center gap-1 pl-2" style={{ borderLeft: `1px solid ${C.gold}20` }}>
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          <span className="text-[14px] font-bold" style={{ color: C.darkGreen }}>
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="flex lg:flex-col lg:w-24 shrink-0 border-b lg:border-b-0 lg:border-r" style={{ borderColor: C.gold, background: C.white }}>
          {TOOLS.map((t) => {
            const Icon = t.icon;
            ...
            return (
              <button
                ...
                style={{ background: active ? "rgba(176,141,69,0.1)" : "transparent", borderRight: active ? `2px solid ${C.gold}` : "2px solid transparent" }}
              >
                <span
                  className="flex h-9 w-9 items-center justify-center rounded-lg"
                  style={{ background: active ? C.gold : C.creamDeep, color: active ? C.darkGreen : C.gray }}
                >
                  <Icon size={17} />
                </span>
                <span className="text-[11px] font-medium" style={{ color: active ? C.darkGreen : C.gray }}>
                  {t.label}
                </span>
              </button>
            );
        ...
        {/* Tool sub-panel */}
        <div className="w-full lg:w-72 shrink-0 p-5" style={{ background: C.white, borderRight: `1px solid ${C.gold}20` }}>
          {/* ---- Box / basket color — separate from the design's own color ---- */}
          <div className="mb-6 pb-6" style={{ borderBottom: `1px solid ${C.gold}20` }}>
            <h3 className="text-[13px] font-bold mb-1" style={{ color: C.ink }}>
              Box Color
            </h3>
            <p className="text-[11.5px] mb-3" style={{ color: C.gray }}>
                ...
                return (
                  <button key={c.id} onClick={() => setBoxColor(c.hex)} title={c.label} className="flex flex-col items-center gap-1">
                    <span
                      ...
                      style={{ background: c.hex, border: active ? `3px solid ${C.gold}` : "3px solid transparent", boxShadow: "0 0 0 1px rgba(14,31,22,0.15)" }}
                    >
                      {active && <Check size={13} style={{ color: c.id === "yellow" ? C.darkGreen : C.cream }} />}
                    </span>
                  </button>
                );
              })}
          ...
          {activeTool === "uploads" && (
            <>
              ...
              <p className="mt-3 text-[12px] leading-relaxed" style={{ color: C.gray }}>
                Or drag and drop an image straight onto the box on the right. PNG or JPG, up to 25MB.
              </p>
              {uploadedImage && (
                <p className="mt-2 text-[12px] leading-relaxed font-medium" style={{ color: C.darkGreen }}>
          ...
          {activeTool === "text" && (
            <>
              ...
              <button className="flex w-full items-center justify-center gap-2 rounded-lg py-3 text-[13.5px] font-semibold" style={{ background: C.creamDeep, color: C.ink }}>
                <Plus size={16} />
                Add text
              </button>
            </>
          ...
          {activeTool === "graphics" && (
            <>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="aspect-square rounded-lg flex items-center justify-center" style={{ background: C.creamDeep }}>
                    <Shapes size={16} style={{ color: C.gray }} />
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-10 relative" style={{ minHeight: "70vh" }}>
          <div className="relative w-full max-w-xl" style={{ paddingTop: 36, paddingRight: 36 }}>
            ...
            <div
              ...
              style={{
                aspectRatio: "16 / 10",
                background: boxColor,
                boxShadow: "0 16px 36px rgba(14,31,22,0.22)",
              }}
            >
              {/* guide labels */}
              ...
              <span className="absolute top-3 right-3 rounded-full px-2.5 py-1 text-[10px] font-bold" style={{ background: "rgba(14,31,22,0.1)", color: C.darkGreen }}>
                Bleed
              </span>
              ...
              <div
                ...
                style={{ border: `2px dashed ${dragOver ? C.gold : "#7FB8D9"}`, background: dragOver ? "rgba(176,141,69,0.12)" : "transparent" }}
              >
                {!uploadedImage ? (
                  ...
                  <div className="relative h-full w-full group flex items-center justify-center">
                    ...
                    {!isDragging && (
                      <span
                        ...
                        style={{ background: "rgba(14,31,22,0.7)", color: C.cream }}
                      >
                        Drag to move
                      </span>
                    )}
                    <div className="absolute top-2 right-2 flex gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        style={{ background: "rgba(14,31,22,0.8)", color: C.cream }}
                      >
                        <Settings size={14} />
                      </button>
                      <button
                        onClick={removeImage}
                        className="flex h-8 w-8 sm:h-7 sm:w-7 items-center justify-center rounded-full"
                        style={{ background: "#7A1F2B", color: C.cream }}
                      >
                        <X size={14} />
                      </button>
                    </div>
            ...
            <div className="mt-3 flex items-center justify-between text-[11px]" style={{ color: C.gray }}>
              <span>3cm</span>
              <span>8cm</span>
            </div>
          </div>
          ...
          <div
            ...
            style={{ background: C.white, boxShadow: "0 4px 16px rgba(14,31,22,0.12)" }}
          >
            <button onClick={() => setZoom((z) => Math.max(50, z - 10))} className="flex h-7 w-7 items-center justify-center rounded-full" style={{ background: C.creamDeep }}>
              <Minus size={13} style={{ color: C.ink }} />
            </button>
            <span className="text-[12.5px] font-semibold w-10 text-center" style={{ color: C.ink }}>
              {zoom}%
            </span>
            <button onClick={() => setZoom((z) => Math.min(200, z + 10))} className="flex h-7 w-7 items-center justify-center rounded-full" style={{ background: C.creamDeep }}>
              <Plus size={13} style={{ color: C.ink }} />
            </button>

            <div className="w-px h-5" style={{ background: C.gold }} />

            <button onClick={rotateLeft} title="Rotate left" className="flex h-7 w-7 items-center justify-center rounded-full" style={{ background: C.creamDeep }}>
              <RotateCcw size={13} style={{ color: C.ink }} />
            </button>
            <span className="text-[12.5px] font-semibold w-9 text-center" style={{ color: C.ink }}>
              {rotation % 360}°
            </span>
            <button onClick={rotateRight} title="Rotate right" className="flex h-7 w-7 items-center justify-center rounded-full" style={{ background: C.creamDeep }}>
              <RotateCw size={13} style={{ color: C.ink }} />
            </button>

            <div className="w-px h-5" style={{ background: C.gold }} />
            <button onClick={resetTransform} title="Reset zoom, rotation & position" className="flex h-7 w-7 items-center justify-center rounded-full" style={{ background: C.creamDeep }}>
        ...
        {showAdjust && uploadedImage && (
          <>
            <motion.div
              ...
              style={{ background: "rgba(10,23,15,0.55)" }}
            />
            <motion.div
            ...
            >
              ...
              <div className="mb-5">
                ...
                <div className="flex items-center gap-2.5">
                  <button
                    ...
                    style={{ background: C.creamDeep, color: C.ink }}
                  >
                    <RotateCcw size={13} />
                    Left
                  </button>
                  ...
                  <span className="text-[12.5px] font-semibold ml-auto" style={{ color: C.gray }}>
                    {rotation % 360}°
                  </span>
                </div>
              </div>
              ...
              {/* Recolor design */}
              <div className="mb-5 rounded-xl p-3.5" style={{ background: C.cream, border: `1px solid ${C.gold}20` }}>
                      ...
                      >
                        <span
                          ...
                          style={{
                            ...
                            boxShadow: "0 0 0 1px rgba(14,31,22,0.15)",
                          }}
                        >
                          {active && <Check size={14} style={{ color: C.cream }} />}
                        </span>
                        <span className="text-[10px] font-medium" style={{ color: active ? C.darkGreen : C.gray }}>
                          {s.label}
                        </span>
                      </button>
                    );
              ...
              <div className="flex items-center gap-2.5">
                <button
                  ...
                  style={{ background: inverted ? C.darkGreen : C.creamDeep, color: inverted ? C.gold : C.ink }}
                >
                  Invert colors
                </button>
                <button
      ...
      <AnimatePresence>
        {showWarning && (
          <motion.div
            ...
            style={{ background: "#7A1F2B", color: C.cream, boxShadow: "0 8px 24px rgba(0,0,0,0.25)" }}
          >
